...
def run_on_board(kernel, constexprs, name):
    ...
    arguments = {"x_ptr": x.tolist(), "y_ptr": y.tolist(), "output_ptr": [0.0] * size, "n_elements": size}
    ...
    result = compile_deploy_and_run(kernel, arguments, grid, f"artifacts/riscv/{name}.elf", RISCV_HOST,
                                     constexprs=constexprs, expected={"output_ptr": output_torch_cpu.tolist()},
...
run_on_board(add_kernel, {"BLOCK_SIZE": CPU_BLOCK_SIZE}, "rvv-vector-add")
